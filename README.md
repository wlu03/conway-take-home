...
2. Oztas et al., [Enhancing Anti-Money Laundering: Development of a Synthetic Transaction Monitoring Dataset](https://ieeexplore.ieee.org/document/10356193), IEEE 2023 — dataset structure and typology definitions. This paper explicitly labels Cycle, Scatter-Gather, Stacked Bipartite, and Layered Fan-In/Out as typologies that require network-level analysis to detect, which directly motivated the graph feature layer in `pipeline/graph_features.py`.
3. Liu et al., [Isolation Forest](https://ieeexplore.ieee.org/document/4781136), ICDM 2008 — scoring method and parameter choices. The paper's recommendation of `max_samples=256` is used directly, as small subsamples reduce the masking effect when anomalies cluster. The `n_estimators=100` cutoff is also from this paper, beyond which returns diminish. Isolation Forest was chosen as the primary scorer because it is non-parametric and handles the multimodal inlier distribution in SAML-D without distributional assumptions.
...
5. Weber et al., [Anti-Money Laundering in Bitcoin: Experimenting with Graph Convolutional Networks for Financial Forensics](https://arxiv.org/abs/1908.02591), KDD Workshop 2019 — empirical evidence that graph features improve AML detection F1 by 30–40% over transaction-level features on layering typologies. This paper was the primary motivation for adding graph-based features (`in_any_cycle`, `scatter_gather_score`, `shared_counterparty_count`) to capture network topology signals that are invisible at the individual transaction level.
6. Savage et al., [Detection of Money Laundering Groups Using Supervised Learning in Networks](https://arxiv.org/abs/1608.00708), 2016 — shows laundering networks have measurably different topological properties (higher clustering coefficient, shorter average path length, more bidirectional edges) than legitimate transaction graphs. This informed the design of `shared_counterparty_count` and cycle detection features as proxies for these topological signals without requiring a full graph library.
