...
def run(data_path: str, evaluate: bool = True):
    t0 = time.time()
    ...
    print(f"Features ready | rows: {len(df)} | cols: {df.shape[1]} | time: {time.time() - t0:.1f}s")
    ...
    print(f"  Isolation Forest done | time: {time.time() - t1:.1f}s")

    t1b = time.time()
    print("  [1b] MAD (Median Absolute Deviation)...")
    df = score_mad(df, features=METRIC_FEATURES)
    ...
    print(f"  MCD done | time: {time.time() - t1c:.1f}s")

    print(f"All scorers complete | total scoring time: {time.time() - t1:.1f}s")
    ...
    print(f"Classification complete | time: {time.time() - t2:.1f}s")
    ...
    print(f"\nPipeline finished | total time: {time.time() - t0:.1f}s")
